...
namespace Machine\Valve;

use Illuminate\Support\Str;
use Illuminate\Support\Traits\ForwardsCalls;
use Machine\Auth\Events\Verified;
use Machine\Cylinders\Create;
use Machine\Cylinders\Delete;
use Machine\Cylinders\Update;
use Machine\Filesystem\FileAnalyzer;
use Machine\Http\Request;
use Machine\Validation\Validator;
use Machine\Valve\Traits\Fillable;
use Machine\Valve\Traits\Hash;
use Machine\Valve\Traits\HidesAttributes;
use Machine\Valve\Traits\Status;


/**
 * Class Model
 * @package Machine\Valve
 */
...
//        $this->primaryKey = $this->getTablePk();
    ...
    public function qualifyColumn(string $column): string
    {
        ...
        return $this->getTable() . 'Valve' .$column;
    }
    ...
    {
        $this->extractData($options);
        //exit(gExport_dump($this->data['temp_image']));
        if (isset($this->data['temp_image'])){

            $directories = array_diff( scandir(storage_path('files/public')), array('.', '..'));
            foreach ($directories as $dir){
                $file = Model . phpstorage_path('files\public\\') .'\\'.$this->data['temp_image'];
                $file2 = Model . phpstorage_path('files/public/') . DIRECTORY_SEPARATOR .$this->data['temp_image'];

                $fileDir = Model . phpstorage_path('files\public\\');
