...
use Machine\Support\ARRAY;
use Machine\Support\FILES;
use PDOException;

/**
 * Class Upload
 * This class performs image, file and media uploads on the system!
 * @package Machine\Cylinders\Helpers
 */
abstract class Uploader
{
    ...
    {
        if (!is_null(config('filesystems.disks.rootFolder')) && !is_null(config('filesystems.disks.siteFolder'))){
            self::$siteDir = str_replace(config('filesystems.disks.rootFolder'), config('filesystems.disks.siteFolder'), root_path());
        }

        self::$storeDir = ((string)$BaseDir ? $BaseDir : config('filesystems.disks.public.root'));
